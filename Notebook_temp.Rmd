...
base::system.time(
  OISST_data <- dl_years %>% 
    ...
    dplyr::ungroup() %>% 
    dplyr::select(lon, lat, t, temp)
) # 740 seconds, ~100 seconds per batch
...
event_only <- function(df){
  ...
  # Then the events
  event <- heatwaveR::detect_event(data = clim, coldSpells = F)
...
) # 31 seconds #### solved  - loplyr and then dplyr
...
OISST_n_total <- OISST_n %>% 
  group_by(lon, lat) %>% 
  mutate(total = sum(n))
head(OISST_n_total)

OISST_n_totalMHW <- OISST_n %>% 
  group_by(lon, lat) %>% 
  mutate(total = sum(n))
head(OISST_n_total)
...
map_slope <- ggplot(OISST_nTrend, aes(x = lon, y = lat)) +
  ...
  scale_fill_gradient2(name = "count/year (slope)",
                       high = "darkblue",
                       mid = "white",
                       low = "blue",
                       midpoint = 0,
  ...
  scale_colour_manual(breaks = c("(0,0.001]", "(0.001,0.01]", "(0.01,0.05]", "(0.05,1]"),
                      values = c("blue1", "blue2", "blue3", "white"),
                      name = "p-value", guide = "none") +
...
# Visualise total number of MCS and MHW

```{r}

map_total<- ggplot(OISST_n_total, aes(x = lon, y = lat)) +
  geom_rect(linewidth = 0.2, fill = NA,
       aes(xmin = lon - 0.1, xmax = lon + 0.1, ymin = lat - 0.1, ymax = lat + 0.1)) +
  geom_raster(aes(fill = total), interpolate = FALSE, alpha = 0.9) +
  geom_polygon(data = map_base, aes(group = group), 
               colour = NA, fill = "grey80")+
  coord_fixed(ratio = 1, xlim = c(-49.0, -47.0), ylim = c(-29, -25), expand = TRUE) +
  scale_fill_distiller(name = "number of MCS",
                       direction = -1,
                       guide = guide_colourbar(direction = "horizontal",
                                               title.position = "top",
                                               n.breaks = 10)) +
...
map_totalMHW<- ggplot(OISST_n_totalMHW, aes(x = lon, y = lat)) +
  geom_rect(linewidth = 0.2, fill = NA,
       aes(xmin = lon - 0.1, xmax = lon + 0.1, ymin = lat - 0.1, ymax = lat + 0.1)) +
  geom_raster(aes(fill = total), interpolate = FALSE, alpha = 0.9) +
  geom_polygon(data = map_base, aes(group = group), 
               colour = NA, fill = "grey80")+
  coord_fixed(ratio = 1, xlim = c(-49.0, -47.0), ylim = c(-29, -25), expand = TRUE) +
  scale_fill_distiller(name = "number of MHW",
                       palette = "Spectral",
                       guide = guide_colourbar(direction = "horizontal",
                                               title.position = "top",
                                               n.breaks = 10)) +
...
map_both_count <- ggpubr::ggarrange(map_total, map_totalMHW, align = "hv")
map_both_count


```
# Creating the  raster layers for modelling

```{r}
#OISST_n_total
OISST_n_total_rst <- OISST_n_total %>% 
  group_by(lon, lat) %>% 
  mutate(total = sum(n)) %>%
  select(lon, lat, total) 
head(OISST_n_total_rst)

# Creating raster marine cold spells
mcs_rst <- rasterFromXYZ(OISST_n_total_rst)
plot(mcs_rst)


# Creating raster marine heat waves
OISST_n_totalMHW <- OISST_n_totalMHW %>% 
  group_by(lon, lat) %>% 
  mutate(total = sum(n))
head(OISST_n_total)
